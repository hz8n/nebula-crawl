#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Spider Nebula — nebula-crawl v1.4.4
...
BANNER = f"""{C}{BOLD}  ⬔⬔⬔  Spider Nebula  ⬔⬔⬔{RST}
{C}  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{RST}
  {W}nebula-crawl • Web Recon • v1.4.4 • Design by al3rab • Full English{RST}
{M}  Link crawler + sitemap + js secret finder (your domain only){RST}
"""

def get_info():
    try: ip=socket.gethostbyname(socket.gethostname())
    except: ip="127.0.0.1"
    return {"host":socket.gethostname(),"ip":ip,"os":platform.system(),"time":datetime.datetime.now().isoformat(),"tool":"nebula-crawl","title":"Spider Nebula","category":"Web Recon","designer":"al3rab","lang":"English"}

def main():
    p=argparse.ArgumentParser(description="Spider Nebula - Link crawler + sitemap + js secret finder (your domain only) | Design by al3rab")
    ...
    args=p.parse_args()
    print(BANNER)
    print(f"{G}[✓] Legendary advanced engine armed{RST}  {GR}nebula-crawl v1.4.4 • Design by al3rab{RST}")
    ...
    if not is_private and not args.allow_public:
        ...
        sys.exit(1)
    data=get_info()
    data.update({"target":args.target,"findings":["Link crawler + sitemap + js secret finder (your domain only) - advanced finding 1","Legendary terminal check passed"],"risk":"LOW","legendary":True,"advanced":True})
